    ...
    if(socket) {
      socket.on('connect', () => {
        ...
        socket.emit('createRoom', {
          roomCode,
          position // Sending both room code and position
        });
      })
          ...
          <div className='flex flex-col gap-3 w-full'>
            {
              socketStatus === 'disconnected' && (
                <div className='flex flex-col gap-6 items-start w-full'>
                  ...
                  <span className='flex gap-1'>
                    <input type="text" value={roomLink} onChange={(e) => setRoomLink(e.target.value)}
                           placeholder="Enter a link"
                           className='bg-gray-300 rounded-md px-4 py-2 outline-none ring-0 text-md font-medium'/>



                    <button className='bg-yellow-400 text-md text-gray-700 font-bold py-2 px-4 rounded-md'
                            onClick={() => {
                              if (roomLink) {
            ...
            }
            {
                socketStatus === 'connected' && roomInfo && (
                    ...
                    <p className='text-md font-bold break-all peer'>{`${window.location.href}location/${roomCode}`}</p>
                    <span className='cursor-pointer p-2 rounded-full  hover:bg-gray-200 flex items-center active:animate-ping' onClick={() => {
                      const url = `${window.location.href}location/${roomCode}`
