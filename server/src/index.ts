...
io.on('connection', (socket: CustomSocket) => {
  ...
  socket.on('createRoom', (data) => {
    ...
    const totalRoomUsers = io.sockets.adapter.rooms.get(roomCode);
    socket.emit('roomCreated', {
      roomCode,
      position: data.position,
      totalConnectedUsers: Array.from(totalRoomUsers || []),
    });
    roomCreator.set(roomCode, socket.id);
    socket.roomId = roomCode; // Attach roomCode to socket
  });
  ...
  socket.on('joinRoom', (data: { roomId: string }) => {
    ...
    if (roomExists) {
      ...
      io.to(socket.id).emit('roomJoined', {
        status: 'OK',
        position: roomCreator.has(data.roomId) ? io.sockets.sockets.get(roomCreator.get(data.roomId)).position : null, // Send the driver's position if available
      });
    } else {
      io.to(socket.id).emit('roomJoined', {
        status: 'ERROR',
  ...
  socket.on('updateLocation', (data) => {
    ...
    if (roomId) {
      // Broadcast location update to all users in the room
      io.to(roomId).emit('updateLocationResponse', { userId: socket.id, position: data.position });
    }
  });
